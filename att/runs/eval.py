...
def evaluate_metric(logger, pred_labels_list, gt_labels_list, label2class_list, test_classes):
    ...
    for c in range(NUM_CLASS):
        ...
        true_positive_num = true_positive_classes[c] + true_positive_num
        gt_classes_num = gt_classes[c] + gt_classes_num
        logger.cprint('----- [class %d]  IoU: %f  Acc: %f-----' % (c, iou, acc))
        acc_list.append(acc)
        iou_list.append(iou)

    OA = true_positive_num / float(gt_classes_num)
    mean_iou = np.array(iou_list[1:]).mean()
    mean_acc = np.array(acc_list[1:]).mean()
    return mean_iou, mean_acc, OA
...
def test_few_shot(test_loader, learner, logger, test_classes, use_zero=False):
    ...
    #print(predicted_label_total, gt_label_total)
    mean_IoU,mean_acc,OA = evaluate_metric(logger, predicted_label_total, gt_label_total, label2class_total, test_classes)
    return mean_loss, mean_IoU,mean_acc,OA
...
def eval(args):
    ...
    learner = Learner(args, mode='test')
    ...
    test_loss, mean_IoU,mean_acc,OA = test_few_shot(TEST_LOADER, learner, logger, TEST_CLASSES)

    logger.cprint('\n=====[TEST] Loss: %.4f | Mean IoU: %f | Mean Acc:%f| OA:%f=====\n' %(test_loss, mean_IoU,mean_acc,OA))
