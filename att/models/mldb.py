...
class ProtoNetAlignQGPASR(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        # prototype learning
        ###全局超点预测支持特征
        support_y1 = support_y.squeeze(1).long().cuda()
        support_y1[:, 1][support_y1[:, 1] == 1] = 2
        loss_ref,_=self.predict(query_prototypes,query_prototype_labels,support_feat.permute(0, 3, 2, 1).reshape(-1, 320),support_y1)
...
##################################
    ...
    def getMutiplePrototypes(self, feat, k):
        ...
        if ratio < 1:
            ...
            #distances = F.pairwise_distance(feat[..., None], farthest_seeds.transpose(0, 1)[None, ...],p=2)  # (n_points, n_prototypes)
            distances =torch.cdist(feat, farthest_seeds, p=2)
    ...
    def calculateLocalConstrainedAffinity(self, node_feat, k=200, method='gaussian'):
        ...
        if method == 'cosine':
            ...
            dist =torch.cdist(node_feat.unsqueeze(1), knn_feat, p=2).squeeze(1)
